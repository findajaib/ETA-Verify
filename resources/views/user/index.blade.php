        ...
        <div class="card collapsed-card m-0">
            <div class="card-header">
                <div class="card-tools">
                    <a href="{{ route('user.create') }}" class="btn btn-primary"><i class="fas fa-user"></i> New User</a>
                </div>
                        ...
                        @foreach ($users as $key => $user)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        @foreach ($user->roles as $role)
                                            <li><a href="{{ route('user.removeRole', ['user' => $user->id, 'role' => $role->id]) }}"
                                                    onclick="confirm('Are You Sure Removing This Role From User?');">{{ $role->name }}</a>
                                            </li>
                                        @endforeach
                                    @endif
                                </td>
                                ...
                                <td>
                                    ...
                                    <a href="#"
                                        onclick="deleteUserFormAction('{{ route('user.destroy', ['user' => $user->id]) }}');"
                                        class="btn btn-danger btn-sm" title="remove user">
                                        <i class="fa fa-trash"></i>
                                    </a>
                                    <a href="#" data-toggle="modal" data-target="#roleModal"
                                        class="btn btn-primary"><i class="fa fa-plus" onclick="getId({{ $user->id }})">
                                            Assign Role</i>
                                    </a>
                                </td>
                            </tr>
